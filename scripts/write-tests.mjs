...
const rfqAndCadCode = `import { test, expect } from '@playwright/test'
...
test.describe('Tier 1 & 2: RFQ Drawer & Architectural CAD / BIM Suite', () => {
  ...
  test.describe('Enquiry Form & Specifier Contact Actions', () => {
    test('validates required fields (Name, Country/Port) before email dispatch', async ({ page }) => {
      ...
      await expect(page.locator('text=/Add your name/i').first()).toBeVisible()
      await expect(page.locator('text=/Add a country/i').first()).toBeVisible()
    })
...
fs.writeFileSync('e2e/rfq-and-cad.spec.ts', rfqAndCadCode, 'utf8')
console.log('Successfully updated e2e/rfq-and-cad.spec.ts')
